...
Admin Portal: For administrative control over doctors, patients, and feedback.
Doctor Portal: For doctors to check and cancel appointments, and manage profiles.
Patient Portal: For patients to register, search for specialized doctors (with real-time highlights), book appointments, and view/cancel schedules.
2. Technical Stack
...
database.txt
 file which was initially drafted for Oracle Database (using sequences like patient, docseq, appoint and Oracle-specific data types like VARCHAR2).

However, the Java connection helper 
ConnectionProvider.java
 is configured for MySQL (using driver com.mysql.cj.jdbc.Driver on port 3306).
...
Apache Tomcat Server: Download and install Tomcat 10 or 10.1.
...
MySQL Server: Ensure MySQL Server is running locally.
Step 2: Set Up MySQL Database
Open your MySQL Command Line Client or GUI (e.g., MySQL Workbench, phpMyAdmin).
Log in as user root with password 1234 (or whatever your local credentials are. If your password is different, remember to edit it in ConnectionProvider.java).
Run the MySQL DDL script provided in Section 5 to create the tables.
Step 3: Configure Database Settings in Code
Navigate to 
ConnectionProvider.java
 and verify the connection string, user, and password:
...
Choose the root directory: DoctorAppointmentSystem and click Finish.
...
Choose your configured Tomcat v10 server.
Click Finish. The application will launch, opening the landing page Home.jsp (e.g., http://localhost:8080/DoctorAppointmentSystem/).
...
When a user logs in (Admin, Doctor, or Patient), the servlet calls request.getSession(true) and stores the user's email as an attribute: session.setAttribute("email", email).
Every protected JSP page contains a scriptlet checking the session: if (session.getAttribute("email") == null) { response.sendRedirect("Login.jsp"); }
We also set HTTP response headers to disable client browser caching of pages:
java
...
During logout, the servlet destroys the session using session.removeAttribute("email") and session.invalidate() before redirecting to the home page."
...
UI Request: The patient selects a doctor and fills out the date/symptoms on PatientHome.jsp, submitting a form targeting /AppointmentReg via a GET request.
